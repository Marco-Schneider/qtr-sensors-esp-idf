...
/* 
  ...
  we're using this approach in order not to have to refactor much of
  what is already known to be working
 */
...
void QTRSensors::emittersOff(QTREmitters emitters, bool wait)
{
      ...
      (_emitterPinCount == 2 && emitters == QTREmitters::Odd))
  {
    ...
    if ((_oddEmitterPin != QTRNoEmitterPin) &&
        (gpio_get_level(static_cast<gpio_num_t>(_oddEmitterPin)) == HIGH))
    {
      ...
      (emitters == QTREmitters::All || emitters == QTREmitters::Even))
  {
    // Check if pin is defined and only turn off if not already off
    if ((_evenEmitterPin != QTRNoEmitterPin) &&
        (gpio_get_level(static_cast<gpio_num_t>(_evenEmitterPin)) == HIGH))
    {
...
void QTRSensors::emittersOn(QTREmitters emitters, bool wait)
{
      ...
      (_emitterPinCount == 2 && emitters == QTREmitters::Odd))
  {
    ...
    // care of this)
    if ((_oddEmitterPin != QTRNoEmitterPin) &&
        ( _dimmable || (gpio_get_level(static_cast<gpio_num_t>(_oddEmitterPin)) == LOW)))
    {
      ...
      (emitters == QTREmitters::All || emitters == QTREmitters::Even))
  {
    ...
    // care of this)
    if ((_evenEmitterPin != QTRNoEmitterPin) &&
        (_dimmable || (gpio_get_level(static_cast<gpio_num_t>(_evenEmitterPin)) == LOW)))
    {
...
{
  if (_dimmable && (gpio_get_level(static_cast<gpio_num_t>(pin)) == HIGH))
  {
    ...
    case QTRType::RC:
...

      {
        ...
        while (time < _maxValue)
        {
          ...
          for (uint8_t i = start; i < _sensorCount; i += step)
          {
            if ((gpio_get_level(static_cast<gpio_num_t>(_sensorPins[i])) == LOW) && (time < sensorValues[i]))
            {
